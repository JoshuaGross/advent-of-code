...
function getCubeValue (cube, z, y, x, w) {
  if (!cube[z]) {
    return false;
  }
  if (y < 0 || x < 0 || w < 0) {
  ...
  if (w >= cube[z][y][x].length) {
    return false;
  }
  return cube[z][y][x][w] === true;
}

function countActiveNeighbors (cube, z, y, x, w) {
  let count = 0;
  for (const dz of range(-1, 1)) {
    for (const dy of range(-1, 1)) {
      for (const dx of range(-1, 1)) {
        for (const dw of range(-1, 1)) {
          if (dz === 0 && dy === 0 && dx === 0 && dw === 0) {
            continue;
          }
          count += (getCubeValue(cube, z+dz, y+dy, x+dx, w+dw)) ? 1 : 0;
        }
      }
    }
  }
...
function getNextCubeValue (cube, z, y, x, w) {
  const activeNeighbors = countActiveNeighbors(cube, z, y, x, w);
  const cubeValue = getCubeValue(cube, z, y, x, w);
...
function cycle (cube) {
  ...
  const maxW = cube[Object.keys(cube)[0]][0][0].length;
  ...
  for (const z of range(minZ, maxZ)) {
    ...
    for (const y of range(-1, maxY)) {
      ...
      for (const x of range(-1, maxX)) {
        const wcol = [];
        for (const w of range(-1, maxW)) {
          wcol.push(getNextCubeValue(cube, z, y, x, w));
        }
        col.push(wcol);
      }
      rows.push(col);
...
module.exports = function runner(inputFile, verbose) {
  ...
  console.log(Object.keys(solution).reduce((sum, k) => {
    return solution[k].reduce((sum2, row) => row.reduce((sum3, col) => col.reduce((sum4, wcol) => sum4 + wcol, sum3), sum2), sum);
  }, 0));
